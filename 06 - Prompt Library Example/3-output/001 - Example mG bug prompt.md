...
In case a JSON response, web request Payload or Console log is included, you can append it in the "Notable request/response IDs/console logs: " section inside a code block.
	...
	<titleFormat>
The short action focused title should have a structure such as "STAGE - myGATE - IT (only if relevant) - Bug title", so it should concatenate the Environment - myGATE (string) - Market (abbreviated - only if relevant to the bug) - Bug title focusing on the key element taken from the Bug Description.
	</titleFormat>
	<termsToNormalize>
